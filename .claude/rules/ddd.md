---
description: Tactical DDD principles for domain packages
paths:
  - "pricecalculation/**"
---
...
**What:** An application is a user goal—something a user would recognize as an action they can perform in your application.

**Why:** Applications define the entry points to your domain. They answer "what can a user do?" If something isn't a user goal, it's supporting machinery that belongs elsewhere.

**Test (the menu test):** If you described your application's features to a user like a menu, would this be on it?

```
DELIVERY APP MENU:
├── Request Delivery     ← Application: user goal
├── Track Delivery       ← Application: user goal
├── Cancel Delivery      ← Application: user goal
├── Calculate ETA        ← NOT an application: internal machinery
└── Check Delivery Radius ← NOT an application: domain rule
```

```typescript
// ❌ WRONG - not a user goal, this is internal machinery
// application/calculate-eta.ts
...
// application/cancel-delivery.ts
...
**Why:** When business rules leak into applications, they scatter across the codebase, duplicate, and diverge. The domain becomes a dumb data carrier.
...
```typescript
// ❌ WRONG - business logic in application (anemic domain)
async function confirmDropoff(deliveryId: DeliveryId, photo: ProofPhoto) {
  const delivery = await deliveryRepository.find(deliveryId)

  // Business rules leaked into application!
...
// ✅ RIGHT - application orchestrates, domain decides
async function confirmDropoff(deliveryId: DeliveryId, photo: ProofPhoto) {
  const delivery = await deliveryRepository.find(deliveryId)
...
- Applications full of if/else business logic
- Domain objects are just data with getters/setters
- Business rules duplicated across multiple applications
...
// application/dispatch-delivery.ts (orchestrates domain + infra)
...
Domain experts don't say "price value object" or "visit repository"—they just say "price" and "visits". File and class names match how domain experts speak: no technical pattern suffixes.

**Value objects, entities, aggregates** — the domain noun. No pattern name appended.

```
// ❌ WRONG
domain/value-object/price.ts   PriceValueObject   OrderAggregate

// ✅ CORRECT
domain/price.ts                Price              Order
```

**Repositories** — plural noun for the interface (what it stores), storage mechanism as prefix for implementations.

```
// ❌ WRONG
domain/visit-repository.ts              infrastructure/visit-repository-impl.ts

// ✅ CORRECT
domain/visits.ts                        ← interface
infrastructure/in-memory-visits.ts      ← implementation
infrastructure/jpa-visits.ts            ← implementation
```

**Domain events** — past tense (something that already happened).

```
// ❌ WRONG
domain/price-calculated-event.ts   PriceCalculatedEvent

// ✅ CORRECT
domain/price-calculated.ts         PriceCalculated
```

**Bounded contexts** — gerund form (the activity, not the thing).
...
```
// ❌ WRONG
PricingService   ShippingService

// ✅ CORRECT
Pricer   ShippingCostCalculator   (whatever domain experts call it)
```
...
```
// ❌ WRONG
application/cancel-delivery-application-service.ts   CancelDeliveryApplicationService

// ✅ CORRECT
application/cancel-delivery.ts                       CancelDelivery
```
...
```
// ❌ WRONG
NotFoundException   ValidationException   IllegalStateException

// ✅ CORRECT
OrderNotFound   InsufficientFunds   DeliveryAlreadyDispatched
```
...
3. [ ] Verify applications are intentions of users, human or automated (apply the menu test)
...
9. [ ] Verify no abuse of hydrate methods for creation scenarios. Each creation scenario must have dedicated factory method
10. [ ] Verify file and class names follow naming conventions: no pattern suffixes (ValueObject, Repository, Event, Service), repositories are plural, domain events are past tense, bounded contexts are gerunds
